...
function Page() {
  ...
  async function generateWallet() {
    ...
    //second endpoint for getting the eoa address
      ...
      // store.setEoa({data2.silentAddress});
      // store eoa
      if(!data2){
        console.log("address api emptyyyyyyy")
      } else{
        console.log("address from api", data2)
      }
      let eoa = {address: data2.silentAddress}
      ...
      {!loading && (
        <>
          <div className="b2-regular p-2 flex rounded-lg border bg-[rgba(96,154,250,0.1)] border-[#1e55af] text-[#1567E4] my-4">
            ...
            <div>
              Connecting your Silent Shard mobile app to your browser generates
              a distributed key.
            </div>
          </div>
